...
class HabitTrackerApp {
    ...
    exitCurrentEditMode(saveChanges = true) {
        if (this.currentlyEditingItem) {
            ...
            // Hide the editable area and show the regular note display if needed
            if (noteEditable) {
    ...
    // Centralized method to enter edit mode, ensuring only one item is editing at a time
    enterEditMode(element, noteEditable, category, itemId) {
        // First, exit any currently editing item
        this.exitCurrentEditMode(true);
        
        // Set up new editing state
        this.currentlyEditingItem = { element, noteEditable, category, itemId };
        
        // Hide any displayed note
        const displayed = element.querySelector('.item-note');
        if (displayed) displayed.style.display = 'none';
        
        // Show and focus the editable area
    ...
    renderCategoryItems(category, containerId, items, dateStr) {
        ...
        items.forEach(item => {
            ...
            const handleEditMode = () => {
                this.enterEditMode(itemEl, noteEditable, category, item.id);
            };
            ...
            const handleItemClick = (ev) => {
                ...
                // Skip if already in edit mode for this specific item (clicking on the editable area of the same item)
                if (clickedElement.closest('.item-note-editable') && 
                    this.currentlyEditingItem && 
                    this.currentlyEditingItem.element === itemEl) {
                    return;
                }
            ...
            // Add click handlers based on status
            if (initialStatus === 'none') {
                ...
                nameEl.addEventListener('click', handleItemClick);
                
                // Also clicking on existing note enters edit mode
                const displayedNote = itemEl.querySelector('.item-note');
                if (displayedNote) {
                    displayedNote.addEventListener('click', handleItemClick);
                }
            }
            ...
            noteEditable.addEventListener('blur', (ev) => {
                // small timeout to allow click on save if any
            ...
            noteEditable.addEventListener('focus', (ev) => {
                if (blurTimeout) {
